...
function plot_sensor_trajectory(pose_sensor, model_pose, tracker_pose, moving, h)
  hold on;
  grid on;
...
function plot_sensor_error(pose_sensor, tracker_pose, moving, h)
  hold on;
  grid on;
  ...
  if moving
    ...
    for i = 1:size(pose_sensor, 1),
      if ishandle(h)
        ...
        set(line3, 'XData', error_sensor(:, 1), 'YData', error_sensor(:, 2));
